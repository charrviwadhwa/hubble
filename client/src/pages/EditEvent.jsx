...
export default function EditSociety() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        triggerHubbleNotif("Society Updated", `${formData.name} configurations have been synchronized.`);
        navigate('/settings');
      } else {
  ...
  const handleDelete = async () => {
    if (!window.confirm("Permanent delete? This will wipe all society data.")) return;
    try {
      const res = await fetch(`http://localhost:3001/api/societies/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      ...
      if (res.ok) {
        triggerHubbleNotif("Society Deleted", "Organization removed from the MSIT hub.");
        navigate('/settings');
      }
        ...
        <main className="flex-1 overflow-y-auto pt-4 pl-4 md:pl-8">
          ...
          <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 pr-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Refine your organization's parameters.</p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => navigate('/settings')} className="rounded-full bg-gray-100 px-6 py-2.5 text-sm font-medium text-gray-700">Cancel</button>
              <button 
                onClick={handleSave}
                disabled={loading}
          ...
          {error && <div className="mb-8 max-w-3xl rounded-xl bg-red-50 p-4 text-sm text-red-600 border border-red-100">⚠️ {error}</div>}
          ...
          <div className="max-w-3xl pr-6 pb-20 space-y-10">
            ...
            <section>
              ...
              <div className="flex items-center gap-6">
                <div className="h-24 w-24 rounded-2xl bg-gray-50 border border-gray-200 flex items-center justify-center overflow-hidden shadow-inner">
                  {preview ? (
                ...
                <div>
                  ...
                  <label htmlFor="logoUpload" className="cursor-pointer text-sm font-medium text-[#ff6b35] hover:underline">Update Logo</label>
                  <p className="mt-1 text-xs text-gray-500">Square PNG or JPG recommended.</p>
                </div>
              </div>
            </section>
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-[150px_1fr] items-center gap-4">
                ...
                <select 
                  className="w-full max-w-md rounded-lg border border-gray-300 px-4 py-2.5 text-sm outline-none focus:border-[#ff6b35] transition-all bg-white"
              ...
              <div className="grid grid-cols-1 md:grid-cols-[150px_1fr] items-start gap-4">
                ...
                <textarea 
                  className="w-full max-w-md h-32 rounded-lg border border-gray-300 px-4 py-3 text-sm outline-none focus:border-[#ff6b35] resize-none transition-all"
            ...
            <section className="space-y-6">
              <h3 className="text-base font-semibold text-gray-900">Administration & Reach</h3>
            ...
            <section className="mt-16 rounded-2xl border border-red-100 bg-red-50/50 p-6">
              ...
              <p className="text-sm text-red-600/70 mb-4">Wiping this society is irreversible. All associated missions and registrations will be terminated.</p>
              <button onClick={handleDelete} className="rounded-lg bg-red-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-red-700 transition-colors">Terminate Society</button>
            </section>
          </div>
        </main>
      </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[150px_1fr] items-center gap-4">
      ...
      <input 
        ...
        value={value} 
        onChange={onChange}
        className="w-full max-w-md rounded-lg border border-gray-300 px-4 py-2.5 text-sm outline-none focus:border-[#ff6b35] focus:ring-1 focus:ring-[#ff6b35] transition-all"
      />
    </div>
  );
}