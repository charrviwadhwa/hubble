...
export default function CreateEvent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        triggerHubbleNotif("Mission Broadcasted", "Your event is now live.");
          ...
          <main className="flex-1 overflow-y-auto pt-4 pl-4 md:pl-8">
            ...
            <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 pr-6">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Publish a new event to the Hubble feed.</p>
              </div>
            </header>
                ...
                <section className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-[150px_1fr] items-start gap-4">
                    ...
                    <div className="w-full max-w-md space-y-3">
                      ...
                      >
                        {refining ? <i className="fi fi-rr-loading animate-spin"></i> : <i className="fi fi-rr-magic-wand"></i>}
                        {refining ? "AI is polishing..." : "Refine with AI"}
                      </button>
                    </div>
                  </div>
                </section>
      ...
      {showPopup && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-3xl bg-white p-8 text-center shadow-2xl animate-in zoom-in-95 duration-300 border border-gray-100">
            ...
            <h2 className="mb-2 text-2xl font-semibold text-gray-900">Event Published!</h2>
            <p className="mb-8 text-sm text-gray-500 leading-relaxed">
              <span className="font-semibold text-gray-800">{formData.title}</span> is now live. Students can start registering immediately.
            </p>
            <button 
